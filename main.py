...
def getForumData():
  num_threads = Post.query.count()
  num_users = User.query.count()
  return (num_threads, num_users)
...
def mserve():
  ...
  data = getForumData()
  return render_template("serve.html", topics=topics, num_users=data[1], num_threads=data[0])
...
def profile(username):
  ...
  if username:
    ...
    user = User.query.get(1) # change to whatever : not logged in + no query
